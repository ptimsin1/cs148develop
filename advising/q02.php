...
if(is-array($records))
{
    foreach($record as $row)
    {
        if($semester != $row ["pmkTerm"])
        {
            ...
            //if end of academic year print closing bracket
            if ($semester  != "" AND ($row ["pmkTearm"] == "Fall"))
            {
                echo "</div>" . LINE_BREAK;
            }
            
            if ($row["pmkTearm"] == "Fall")
            {
              print "<div class='academicYear clearFloats'>";
              
              print '<section class=fourColums ';
              print $row ["pmkTearm"]; 
              //keep the academic year together
           ...
           print '<li ckass ="' . $row["fldRequirements"] . '">'; 
           print $row["fldRequirements"] . " " . $row [["fldCourseNumber"]]; 
           print '</li' . LINE_BREAK ; 
